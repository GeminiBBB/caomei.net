...
//	}
	...
	oLeftBtn.onclick = function () {
		...
		if(iIndex < 0) {
			...
			oList.style.width = - (aListLi.length - 1) * iPerWidth + 'px';
		}
		...
		bufferMove(oList, {width: - iIndex * iPerWidth});
	...
	aBtnListA.forEach(function (v, k) {
		v.index = k;
			console.log(aBtnListA.length)
		v.onclick = function () {
			iIndex = this.index;
			iBtnIndex = this.index;

			bufferMove(oList, {width: - this.index * iPerWidth});
